...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/insert-mongo.sh <dump-file.gz>

Description:
  Restores a gzipped MongoDB dump by running mongorestore inside the Docker mongo service.
  The archive stays on the host and is streamed into the container.
...
Overrides:
  - COMPOSE_FILE_PATH=/abs/path/docker-compose.local.yml
  - MONGO_SERVICE=mongo
  - MONGO_DB_NAME=my_database
  - MONGO_AUTH_DB=admin

Examples:
  ./scripts/insert-mongo.sh dump.gz
  COMPOSE_FILE_PATH=./docker-compose.local.yml ./scripts/insert-mongo.sh backups/prod.gz

Notes:
  - Uses --drop, so collections from the archive replace existing collections in the target database.
EOF
}
...
if [[ ! -f "$archive_file" ]]; then
  echo "Error: dump file not found: $archive_file" >&2
  exit 1
fi
...
echo "Restoring MongoDB dump into Docker service '$mongo_service' from $archive_file"
"${compose_cmd[@]}" exec -T \
...
echo "Restore completed successfully from: $archive_file"
